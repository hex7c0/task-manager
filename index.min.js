...
function wrapperTCP(my) {
    ...
    var server = net.createServer(function(sock) {
        ...
        return my.auth ? grant ? sock.write("> hello master\n", function() {
            ...
            for (var i = 0; ii > i; ++i) {
                var m = mod[keys[i]];
                if (m.regex.test(command) === !0) return m.body(sock, command, workers, next);
            }
...
function wrapperUDP(my) {
    ...
    var server = dgram.createSocket("udp4", function(buff, sock) {
        sock.server = server;
        for (var index, command = String(buff), workers = cluster.workers, i = 0; ii > i; ++i) {
            var m = mod[keys[i]];
            if (m.regex.test(command) === !0) return m.body(sock, command, workers, next);
        }
        return my.custom && my.custom.test(command) ? void my.callback(sock, command) : exitRegex.test(command) ? (write("> exit\n", sock, function() {
            return server.close(function() {
                return output("exit");
            });
        ...
        index += "  kill [pid]\n", index += "  memory\n", index += "  ps\n", index += "  title [name]\n", 
        index += "  uptime\n", index += "  exit\n", index += "  close\n", write(index, sock)) : nyanRegex.test(command) ? (index = "-_-_-_-_-_-_-_,------,      o      \n", 
        index += "_-_-_-_-_-_-_-|   /\\_/\\            \n", index += "-_-_-_-_-_-_-~|__( ^ .^)  +     +  \n", 
        index += '_-_-_-_-_-_-_-""  ""               \n', write(index, sock)) : write('> unrecognized, try "help"\n', sock, null, server);
    });
    return server.on("error", function(e) {
